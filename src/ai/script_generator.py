...
def _format_sanctions(sanctions: list[dict[str, Any]]) -> str:
    """
    制裁情報をプロンプト用テキストに変換する（全件・件数制限なし）。
...
def generate_jra_sanctions_script(sanctions: list[dict[str, Any]]) -> str:
    """
    JRA制裁情報のみのナレーション原稿を生成する。
    全制裁を漏れなく読み上げる構成。制裁件数に応じて字数を調整する。
    """
    today = datetime.now(JST).strftime("%m月%d日")
    logger.info("[INFO] JRA制裁情報動画の原稿生成を開始")

    prompt = f"""以下の{today}のJRA制裁情報をもとに、YouTube Shorts用のナレーション原稿を作成してください。

【制裁情報（全件）】
{_format_sanctions(sanctions)}

【原稿構成（必ず守ること）】
1. オープニング（3秒）: 「本日のJRA制裁情報をお届けします！」から始める
2. 制裁情報（全件・件数に応じた秒数）:
   - 全ての制裁情報を一件ずつ読み上げること（省略禁止）
   - 競馬場名・レース番号・騎手名・馬名・制裁内容を必ず読み上げること
   - 例）「○○競馬場○Rで○○騎手が騎乗した○○に、○○の制裁が科されました」
3. クロージング（5秒）: 「以上、本日のJRA制裁情報でした！チャンネル登録よろしく！」で締める

制裁件数に応じて合計字数を調整し、話し言葉の原稿のみ出力してください（余計な説明文は不要）。"""

    result = _call_ai(prompt)
    if result:
        logger.info("[INFO] JRA制裁情報原稿生成成功")
        return result

    logger.warning("[WARNING] AI生成失敗 → フォールバック制裁原稿を使用")
    return _fallback_jra_sanctions_script(today, sanctions)


def generate_jra_news_script(news: list[dict[str, Any]]) -> str:
    """
    JRA「今日の出来事」のみのナレーション原稿を生成する（60秒以内・約350文字）。
    """
    today = datetime.now(JST).strftime("%m月%d日")
    logger.info("[INFO] JRAニュース動画の原稿生成を開始")

    prompt = f"""以下の{today}のJRA「今日の出来事」をもとに、YouTube Shorts用のナレーション原稿を作成してください。
...
2. 出来事紹介（50秒分）:
   - 競走除外・競走中止・記録達成など話題性の高い内容を重点的に紹介する
   - 競馬場名・レース番号・馬名・内容を具体的に読み上げること
...
合計350文字程度で、話し言葉の原稿のみ出力してください（余計な説明文は不要）。"""
    ...
    result = _call_ai(prompt)
    if result:
        logger.info("[INFO] JRAニュース原稿生成成功")
        return result

    logger.warning("[WARNING] AI生成失敗 → フォールバックニュース原稿を使用")
    return _fallback_jra_news_script(today, news)


def _fallback_jra_sanctions_script(today: str, sanctions: list[dict[str, Any]]) -> str:
    """AI生成失敗時の制裁情報専用フォールバック原稿。全件を読み上げる。"""
    parts = ["本日のJRA制裁情報をお届けします！", "本日の制裁情報です。"]
    ...
    for s in sanctions:
        ...
        horse_str = f"が騎乗した{horse}" if horse else ""
        parts.append(f"{venue}{race}で{jockey}騎手{horse_str}に{content}の制裁。")

    parts.append("以上、本日のJRA制裁情報でした！チャンネル登録よろしく！")
    return "　".join(parts)
