"""
NAR（地方競馬）YouTube Shorts 自動投稿 エントリーポイント

実行フロー:
  1. NAR開催日判定 → 開催なしなら終了
  2. NARスクレイピング（制裁情報 + ニュース）
  3. AI原稿生成
  4. VOICEVOX音声生成
  5. 動画生成
...
from src.ai.script_generator import generate_nar_script
from src.scraper.nar_scraper import get_news, get_sanctions
from src.tts.voicevox_tts import text_to_speech
from src.uploader.youtube_uploader import (
    NAR_TAGS,
    build_nar_description,
    build_nar_title,
    upload_video,
)
from src.utils.calendar import is_nar_race_day
...
def parse_args() -> argparse.Namespace:
    ...
    parser = argparse.ArgumentParser(
        description="地方競馬（NAR）情報をYouTube Shortsに自動投稿するスクリプト"
    )
    parser.add_argument(
        "--dry-run",
        action="store_true",
...
def main() -> int:
    ...
    logger.info("=" * 60)
    logger.info("[INFO] NAR地方競馬 YouTube Shorts 自動投稿 開始")
    ...
    # ========== Step 1: NAR開催日判定 ==========
    if not args.force:
        logger.info("[INFO] Step 1: NAR開催日判定")
        if not is_nar_race_day():
            logger.info("[INFO] 本日はNAR開催なし。処理を終了します。")
            return 0
        logger.info("[INFO] 本日はNAR開催日です。処理を続行します。")
    else:
    ...
    try:
        # ========== Step 2: NARスクレイピング ==========
        logger.info("[INFO] Step 2: NARスクレイピング開始")
        ...
        if not dry_run:
            ...
            title = build_nar_title(date_str)
            description = build_nar_description(script)
